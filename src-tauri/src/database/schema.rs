...
impl Database {
    ...
    fn migrate_v1_to_v2(conn: &Connection) -> Result<(), AppError> {
        // 某些早期版本/分支可能已经把 user_version 标记为 1，但实际表结构仍缺少
        // v0 -> v1 应补齐的列（例如 skills.installed_at）。先做一次幂等补齐，
        // 兼容这类“版本号已前进、列结构未完全同步”的旧库。
        Self::migrate_v0_to_v1(conn)?;
    ...
    fn migrate_skills_table(conn: &Connection) -> Result<(), AppError> {
        ...
        let legacy_has_installed_at = Self::has_column(conn, "skills_old", "installed_at")?;
        let select_sql = if legacy_has_installed_at {
            "SELECT key, installed, installed_at FROM skills_old"
        } else {
            "SELECT key, installed, 0 AS installed_at FROM skills_old"
        };
