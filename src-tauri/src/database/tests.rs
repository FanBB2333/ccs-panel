...
#[test]
fn schema_migration_supports_v1_skills_without_installed_at() {
    let conn = Connection::open_in_memory().expect("open memory db");
    conn.execute_batch(LEGACY_SCHEMA_SQL)
        .expect("seed old schema without installed_at");
    Database::set_user_version(&conn, 1).expect("set legacy version");

    conn.execute(
        "INSERT INTO skills (key, installed) VALUES (?1, ?2)",
        params!["claude:test-skill", true],
    )
    ...
    assert!(
        Database::has_column(&conn, "skills", "enabled_claude").expect("check final skills schema"),
        "skills table should be upgraded to unified v3 schema"
    );
    ...
    let pending: String = conn
        ...
        .expect("read pending migration flag");
    ...
    assert!(
        snapshot.contains("\"directory\":\"test-skill\""),
        "snapshot should preserve migrated directory: {snapshot}"
    );
    assert!(
        snapshot.contains("\"app_type\":\"claude\""),
        "snapshot should preserve migrated app type: {snapshot}"
    );
