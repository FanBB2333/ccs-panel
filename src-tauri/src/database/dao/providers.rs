...
impl Database {
    ...
    /// 兼容旧的 ProviderStore 接口。
    ///
    /// v3.12.3 之后本地数据库已不再维护独立的 proxy target 字段，
    /// 远程 SSH store 仍保留该概念，因此本地实现保持空操作即可。
    pub fn set_proxy_target_provider(&self, _app_type: &str, _id: &str) -> Result<(), AppError> {
        Ok(())
    }
