...
impl ProxyService {
    ...
    pub async fn is_remote_takeover_active(&self) -> bool {
        self.current_server_id.read().await.is_some()
    }
    ...
    /// 启动代理服务器（兼容 ccs-panel 的远程服务器模式）
    ///
    /// - `None`: 使用本地代理接管流程
    /// - `Some(server_id)`: 通过 SSH 远程端口转发改写远端 Claude 配置
    pub async fn start_with_takeover_for_server(
        &self,
        server_id: Option<String>,
    ) -> Result<ProxyServerInfo, String> {
        let Some(server_id) = server_id else {
            return self.start_with_takeover().await;
        };

        if self.is_running().await {
            return Err("本地代理服务正在运行，请先关闭本地代理后再启用远程代理".to_string());
        }

        let existing_server_id = self.current_server_id.read().await.clone();
        if let Some(existing_server_id) = existing_server_id {
            if existing_server_id == server_id {
                if let Some(port) = *self.ssh_forward_port.read().await {
                    return Ok(ProxyServerInfo {
                ...
                self.stop_with_restore_for_server().await?;
            }
        }
        ...
        let ssh_service = self
            .ssh_service
            .as_ref()
            .ok_or_else(|| "远程服务器代理需要 SSH 服务支持".to_string())?;
        ...
        ssh_service
            .start_remote_port_forwarding_to_target(&server_id, remote_port, &target_address)
            .await
            .map_err(|e| format!("SSH 端口转发启动失败: {e}"))?;

        {
            let mut current_server_id = self.current_server_id.write().await;
            *current_server_id = Some(server_id.clone());
        }
        {
            let mut ssh_forward_port = self.ssh_forward_port.write().await;
            *ssh_forward_port = Some(remote_port);
        }
        ...
        if let Err(e) = self
            .update_remote_configs_for_direct_proxy(&server_id, remote_port)
            .await
        {
            let _ = ssh_service.stop_port_forwarding(&server_id).await;
            let _ = self.db.set_live_takeover_active(false).await;
            *self.current_server_id.write().await = None;
            *self.ssh_forward_port.write().await = None;
            return Err(e);
        }
    ...
    fn extract_base_url_from_provider(&self, provider: &Provider) -> Result<String, String> {
    ...
    /// 更新远程服务器的 Claude 配置，令其经由 SSH 隧道访问当前 Provider。
    async fn update_remote_configs_for_direct_proxy(
        ...
        let ssh_service = self
            .ssh_service
            .as_ref()
            .ok_or_else(|| "SSH 服务未初始化".to_string())?;
        ...
        let update_script = format!(
            ...
            if [ -f "$SETTINGS" ]; then
                ...
                if command -v jq >/dev/null 2>&1; then
                    jq '.env.ANTHROPIC_BASE_URL = "{proxy_url}"' "$SETTINGS" > "$SETTINGS.tmp" && mv "$SETTINGS.tmp" "$SETTINGS"
...
"
                    ...
                    echo '{{"env":{{"ANTHROPIC_BASE_URL":"{proxy_url}"}}}}' > "$SETTINGS"
                    echo "updated_fallback"
                fi
            else
                echo '{{"env":{{"ANTHROPIC_BASE_URL":"{proxy_url}"}}}}' > "$SETTINGS"
                echo "created_new"
        ...
        ssh_service
            .execute(server_id, &update_script)
            .await
            .map(|_| ())
            .map_err(|e| format!("更新远程配置失败: {e}"))
    }

    /// 停止远程服务器代理并恢复远端配置
    pub async fn stop_with_restore_for_server(&self) -> Result<(), String> {
        let server_id = self.current_server_id.read().await.clone();
        ...
        if let Err(e) = self.restore_remote_configs(&server_id).await {
            log::warn!("[ProxyService] 恢复远程配置失败: {e}");
        }

        if let Err(e) = ssh_service.stop_port_forwarding(&server_id).await {
            log::warn!("[ProxyService] 停止 SSH 端口转发失败: {e}");
        }
    ...
    async fn restore_remote_configs(&self, server_id: &str) -> Result<(), String> {
        ...
        ssh_service
            .execute(server_id, restore_script)
            .await
            .map(|_| ())
            .map_err(|e| format!("恢复远程配置失败: {e}"))
    }
    ...
    pub async fn stop_with_restore(&self) -> Result<(), String> {
        if self.is_remote_takeover_active().await {
            return self.stop_with_restore_for_server().await;
        }
    ...
    pub async fn is_takeover_active(&self) -> Result<bool, String> {
        ...
        if status.claude || status.codex || status.gemini {
            return Ok(true);
        }
    ...
    pub async fn get_status(&self) -> Result<ProxyStatus, String> {
        if let Some(server) = self.server.read().await.as_ref() {
        ...
        } else if let Some(port) = *self.ssh_forward_port.read().await {
            let current_provider = self
                ...
                .and_then(|provider_id| {
                    self.db
                        ...
                        .ok()
                        .flatten()
                        .map(|provider| (provider_id, provider.name))
                });

            let active_targets = current_provider
                .as_ref()
            ...
            Ok(ProxyStatus {
                ...
                address: "127.0.0.1".to_string(),
                port,
                current_provider: current_provider.as_ref().map(|(_, name)| name.clone()),
                current_provider_id: current_provider.map(|(id, _)| id),
                active_targets,
                ..Default::default()
            })
        } else {
