...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&ccs_panel_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text =
        std::fs::read_to_string(ccs_panel_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
